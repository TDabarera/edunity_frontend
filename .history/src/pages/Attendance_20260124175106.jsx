...
import { Toast, AttendanceRecords, Banner } from '../components/organisms';
...
const Attendance = () => {
  ...
  const attendanceColumns = [
    { label: 'Student Name & Account Number', align: 'left' },
  ];
  ...
  return (
    <MainLayout isLoggedIn={isLoggedIn} user={user}>
      <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        {/* Banner: Select Class to Begin */}
        {!selectedClass && (
          <Banner
            heading="Select a Class to Begin"
            description="Please choose a class from the dropdown above to view students and manage attendance records."
            backgroundColor={colors.bannerBg}
            textColor={colors.text.primary}
            avatarImage="Characters/DidYouKnow.png"
          />
        )}

        {/* Mark Attendance Button */}
        {selectedClass && !showAttendanceForm && (
        ...
        {selectedClass && (
          <Box sx={{ mt: 4 }}>
            <AttendanceRecords 
              classId={selectedClass}
              onError={(message, severity) => showToast(message, severity || 'error')}
            />
          </Box>
        )}
