...
import { GetAttendanceByClass, UpdateAttendanceRecord, DeleteAttendanceRecord } from '../../services';
...
const AttendanceRecords = ({ classId, onError }) => {
  ...
  const [dateGroupToDelete, setDateGroupToDelete] = useState(null);
  ...
  const fetchAttendanceRecords = async () => {
    console.log('[DEBUG AttendanceRecords] fetchAttendanceRecords called with classId:', classId);
    
    if (!classId) {
      console.log('[DEBUG AttendanceRecords] No classId, clearing records');
      setRecords([]);
      setGroupedRecords([]);
      return;
    }
    ...
    try {
      setLoading(true);
      console.log('[DEBUG AttendanceRecords] Calling GetAttendanceByClass with:', classId);
      const response = await GetAttendanceByClass(classId);
      console.log('[DEBUG AttendanceRecords] API Response:', response);
      
      const attendanceData = response.attendance || [];
      console.log('[DEBUG AttendanceRecords] Attendance data count:', attendanceData.length);
      ...
      console.log('[DEBUG AttendanceRecords] Grouped array count:', groupedArray.length);
  ...
  useEffect(() => {
    console.log('[DEBUG AttendanceRecords] useEffect triggered, classId:', classId);
  ...
  // Handle delete attendance record - show confirmation popup
  const handleDeleteClick = (dateGroup) => {
    setDateGroupToDelete(dateGroup);
  ...
  const confirmDelete = async () => {
    ...
    if (!dateGroupToDelete) return;

    try {
      setLoading(true);
      // Delete all records for this date
      const deletePromises = dateGroupToDelete.records.map(record => 
      ...
      if (onError) {
        onError('Attendance records deleted successfully', 'success');
      }
    } catch (error) {
      if (onError) {
        onError(error.message || 'Failed to delete attendance records');
      }
    } finally {
      setLoading(false);
      setDateGroupToDelete(null);
    }
  };
  ...
  return (
    <Paper elevation={2} sx={{ p: 3 }}>
                    ...
                    <TableCell align="center">
                      ...
                      <Tooltip title="Delete Attendance">
                        <IconButton
                          size="small"
                          color="error"
                          onClick={() => handleDeleteClick(dateGroup)}
                        >
      ...
      <Popup
        ...
        description={`Are you sure you want to delete attendance records for ${dateGroupToDelete ? formatDate(dateGroupToDelete.date) : ''}? This action cannot be undone.`}
      ...
      <Popup
        ...
        description="Are you sure you want to update these attendance records?"
