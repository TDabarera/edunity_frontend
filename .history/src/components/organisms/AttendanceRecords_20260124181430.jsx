...
const AttendanceRecords = ({ classId, onError }) => {
  ...
  const fetchAttendanceRecords = async () => {
    ...
    try {
      ...
      // WORKAROUND: Backend is not filtering by classId, so filter on frontend
      const filteredData = attendanceData.filter(record => {
        const recordClassId = record.className || record.classId || record.class;
        return recordClassId === classId;
      });
      
      console.log('[DEBUG AttendanceRecords] Filtered data count:', filteredData.length);
