...
const AttendanceRecords = ({ classId, onError }) => {
  ...
  const handleDeleteClick = async (dateGroup) => {
    if (!window.confirm(`Are you sure you want to delete attendance records for ${formatDate(dateGroup.date)}?`)) {
      return;
    }
    ...
    try {
      ...
      const deletePromises = dateGroup.records.map(record => 
