  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter children - students whose parentId matches this parent's userId
        // or whose children array includes this parent's userId
        const childrenData = allUsers.filter(u => {
          const isStudent = (u.userType || '').toLowerCase() === 'student';
          const hasParentId = u.parentId === parentUserId || u.parent === parentUserId;
          const inChildrenArray = u.children && Array.isArray(u.children) && u.children.includes(parentUserId);
          return isStudent && (hasParentId || inChildrenArray);
        });
        
        console.log('[ParentChildren] All users count:', allUsers.length);
        console.log('[ParentChildren] Filtered children:', childrenData);
